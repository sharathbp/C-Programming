...
int main()
{
	...
	int i,j,x,y,area;
	void *p;
	initgraph(&gd,&gm, driver); //initialize the graphics mode
	...
	line(minx+275,maxy-100,minx+275,maxy-30);
	line(maxx-276,maxy-100,maxx-276,maxy-30);
	arc(minx+319,maxy-90,-10,45,44);
	...
	line(minx+306,maxy-62,minx+306,maxy-30);
	line(maxx-306,maxy-62,maxx-306,maxy-30);
	arc(minx+319,maxy-60,0,180,13);

	line(minx+306,maxy-105,minx+306,maxy-80);    	//above part
	line(maxx-307,maxy-105,maxx-307,maxy-80);
	...
	arc(minx+319,maxy,70,110,200);
	arc(minx+319,maxy,70,110,197);
	...
	for(i=0;i<2;i++){
		...
		line(abs(minx+204),maxy-52,abs(minx+204),maxy-30);
		line(abs(minx+244),maxy-52,abs(minx+244),maxy-30);
		...
		rectangle(abs(minx+204),maxy-160,abs(minx+244),maxy-130); //top part
		line(abs(minx+214),maxy-147,abs(minx+214),maxy-130);
		line(abs(minx+234),maxy-147,abs(minx+234),maxy-130);
		...
		line(abs(minx+150),maxy-125,abs(minx+195),maxy-130);   	// separation
		line(abs(minx+150),maxy-73,abs(minx+195),maxy-78);

		line(abs(minx+150),maxy-122,abs(minx+195),maxy-127);	// separation
		line(abs(minx+150),maxy-70,abs(minx+195),maxy-75);

		line(abs(minx+160),maxy-102,abs(minx+160),maxy-74);
		line(abs(minx+184),maxy-104,abs(minx+184),maxy-74);
		arc(abs(minx+172),maxy-102,-7,190,12);

		line(abs(minx+164),maxy-82,abs(minx+180),maxy-84);		// solid rectangle
		...
		line(abs(minx+160),maxy-50,abs(minx+160),maxy-30);
		line(abs(minx+184),maxy-52,abs(minx+184),maxy-30);
		arc(abs(minx+172),maxy-50,-7,190,12);
		...
		for(j=0;j<2;j++){
			...
			line(abs(minx+100),maxy-162,abs(minx+100),maxy-151);
			line(abs(minx+110),maxy-162,abs(minx+110),maxy-151);
			arc(abs(minx+105),maxy-160,0,180,5);

			bar(abs(minx+90),maxy-171,abs(minx+120),maxy-170);
			arc(abs(minx+105),maxy-175,-20,200,12);
